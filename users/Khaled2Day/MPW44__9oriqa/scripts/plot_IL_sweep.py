...
def plot(test):
    ...
    if YLIM:
        ax.set_ylim(*YLIM)
    # else: leave the y-axis fully auto-scaled by matplotlib from the real data --
    # no forced top/bottom. Different files can have genuinely different Y ranges
    # (e.g. values above 0 dB from noise or a calibration offset), and clamping
    # the top to exactly 0 would silently cut that data out of view.
